...
### 4.2 GitHub Release 必须存在

- ⚠️ **开发模式限制**：在 `npm run tauri dev` 开发模式下，updater 功能会因为 GitHub Release endpoint 不存在而失败
- ⚠️ **首次运行限制**：首次推送到 main 之前，`latest.json` endpoint 返回 404，手动检查更新会失败
- ✅ **错误处理**：已添加友好提示：「GitHub Release 尚未配置，请先推送代码触发 CI 构建」
- ✅ **自动更新静默失败**：自动更新检查失败时不显示错误，避免干扰用户
